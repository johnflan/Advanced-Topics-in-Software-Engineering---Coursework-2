...
public class BillingSystemTest {
	...
	public void checkWhatHappensIfSomeoneCallsHimself(){
		...
		context.checking(new Expectations(){{
			...
			oneOf (billGenerator).send(with(customer1), with(any(List.class)), with(expectedCost));
		}});
	...
	public void checkWhatHappensIfTwoCustomersHaveTheSamePhoneNumber(){
		...
		context.checking(new Expectations(){{
			...
			oneOf (billGenerator).send(with(customer1), with(any(List.class)), with(expectedCost1));
			oneOf (billGenerator).send(with(customer1b), with(any(List.class)), with(expectedCost1b));
		}});
	...
	public void checkCallCostWhenCallIsInPeakTime(){
		...
		context.checking(new Expectations(){{
			...
			oneOf (billGenerator).send(with(customer1), with(any(List.class)), with(expectedCost1));
			oneOf (billGenerator).send(with(customer2), with(any(List.class)), with(expectedCost2));
			oneOf (billGenerator).send(with(customer3), with(any(List.class)), with(expectedCost3));
		}});
